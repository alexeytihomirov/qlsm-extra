...
qlmatch-packer addon (it was split out of this addon's own
ansible_instance_demos.py in the first place, then hoisted into core so both
could reach it). Duplicated instead of kept shared once it became clear the
addon system has no way to declare or enforce a dependency between addons --
see addons/README.md's "Where the line falls" in qlsm for the full reasoning,
which applies here identically (telemetry-relay's and demo-stream's private
copies of the old ui/stats_hub.py are the same pattern). This copy and
qlmatch-packer's are free to diverge from here on.
"""
import logging
import os
